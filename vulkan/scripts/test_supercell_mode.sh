#!/usr/bin/env bash
# Supercell mode smoke test for Vulkan volume rendering.
# Auto-detects available export fields, prefers key storm fields, and
# launches the viewer with natural texture detail plus orbit camera settings.
set -euo pipefail
...
echo "[supercell] input=${INPUT_DIR} fields=${FIELDS_CSV}"
exec "${BIN}" \
  --window-test \
